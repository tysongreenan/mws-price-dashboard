    ...
    "fuelSurcharges": {
      ...
      "average": 16.5
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Filter to only include locations where Miller Waste operates
            // Check if Miller has at least one bin size available in that location
            const locationsWithMiller = uniqueLocs.filter(loc => {
                const hasMiller14 = getMillerPrice(loc, 'p14') !== null;
                const hasMiller20 = getMillerPrice(loc, 'p20') !== null;
                const hasMiller40 = getMillerPrice(loc, 'p40') !== null;
                return hasMiller14 || hasMiller20 || hasMiller40;
            });

            const sortedLocations = locationsWithMiller.sort((a, b) => {
            ...
            chartDatasets.forEach((dataset, index) => {
                ...
                // Color indicator - make it larger and more visible
                const colorBox = document.createElement('span');
                colorBox.className = 'inline-block rounded-sm';
            ...
            // Print optimization function - make it globally accessible
            window.prepareForPrint = function() {
                try {
                    // Store all chart instances if they exist globally
                    const chartInstances = [];
                    
                    // Find all canvas elements and ensure their charts are rendered
                        ...
                        canvasElements.forEach(canvas => {
                            try {
                                ...
                                if (chart) {
                                    ...
                                    // Special handling for Regional Price Analysis chart
                                    if (canvas.id === 'locationPriceChart') {
                                        // Show all datasets for print
                    ...
                    // Fallback to direct print if something goes wrong
                    window.print();
                }
            };

            // Attach print button event listener as backup (onclick should work, but this ensures it)
            const exportButton = document.getElementById('export-report-btn');
            if (exportButton) {
                exportButton.addEventListener('click', function(e) {
                    e.preventDefault();
                    if (window.prepareForPrint) {
            ...
            // Also handle browser print dialog (Ctrl/Cmd+P)
                ...
                if (typeof Chart !== 'undefined' && Chart.getChart) {
                    canvasElements.forEach(canvas => {
                        try {
                            ...
                            console.warn('Error updating chart for print:', e);
                        }
                    });
                }
            });
